...
const TopBar = () => {
  ...
  // Single "Tasks" entry — Create/Search/View/Edit/Delete all live inside
  // the Tasks module page itself, so the sidebar doesn't need separate
  // entries (or a page navigation) for each action.
  const links = [
    { path: "/", label: "Home" },
  ...
  // Collapse the quick-search bar whenever the route changes. The
  // context itself also clears searchTerm on route change, so this
  // just resets the local "is it expanded" UI state.
  useEffect(() => {
  ...
  return (
    <div>
      ...
      {isDesktop ? (
        // Desktop reveals the sidebar on hover, so keep an invisible
        // trigger strip at the edge instead of showing the mobile hamburger.
        <div
