...
const TaskList = () => {
  ...
  // Search state — shared with the topbar so typing there filters this
  // page live, no navigation or URL param involved.
  ...
  // Derive the active task from the live list (instead of a static
  // snapshot) so the panel always reflects the latest saved data —
  // e.g. right after an edit is saved, or if a subtask is toggled.
  ...
  // Arriving via /tasks?add=1 (e.g. the footer's + button, or any other
  // shortcut) opens straight into the Add Task panel on this same page
  // instead of a dedicated /tasks/new route.
  useEffect(() => {
  ...
  // View / edit / add all stay on this page and open an inline panel —
  // no navigation to a separate route, on desktop or mobile.
  ...
  const handleAddTaskSubmit = async (e) => {
    ...
    // Stay on the Add Task form, cleared and ready for the next entry.
    setInputValue("");
  ...
  const handleSaveEdit = async (taskId, newPriority, newDeadline) => {
    ...
    // Stay on the Edit form — activeTask is derived live from tasks, so
    // it now reflects the just-saved priority/deadline.
  };
  ...
  return (
    <div className="desktop-container">
      <div className={`desktop-content ${sidePanelMode ? "layout-split" : ""}`}>
        ...
        {/* Right Side: Sticky Split Pane on desktop, stacks in-place on mobile */}
        {sidePanelMode && (
