...
type PillTone = "accent" | "muted" | "success" | "danger";
...
export function DotBadge({ tone = "muted", live = false, children, className = "" }: DotBadgeProps) {
  return (
    <span
      className={`inline-flex items-center gap-[0.35rem] whitespace-nowrap text-[0.78rem] font-semibold ${TONE_TEXT[tone]} ${className}`}
    >
      <span
        className={`h-[0.4rem] w-[0.4rem] bg-current ${live ? "overdrive-live-dot" : "rounded-full"}`}
      />
      {children}
    </span>
  );
...
export function PillBadge({ tone = "muted", children, className = "" }: PillBadgeProps) {
  return (
    <span
...
interface RevokedStampProps {
  children?: ReactNode;
  className?: string;
}

export function RevokedStamp({ children = "Revoked", className = "" }: RevokedStampProps) {
  return (
    <span
