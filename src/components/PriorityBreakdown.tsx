...
/*
 * PriorityBreakdownTask mirrors the shape returned by useTasks rather than a
 * hand-written interface with its own optionality, so passing `tasks` from
 * useTasks() directly always type-checks.
 */
export type PriorityBreakdownTask = Pick<
  ReturnType<typeof useTasks>["tasks"][number],
  "completed" | "priority"
>;

interface PriorityBreakdownProps {
  tasks: PriorityBreakdownTask[];
}

const PriorityBreakdown = ({ tasks }: PriorityBreakdownProps) => {
  const incomplete = tasks.filter((task) => !task.completed);
  const counts = incomplete.reduce<Record<number, number>>((acc, task) => {
